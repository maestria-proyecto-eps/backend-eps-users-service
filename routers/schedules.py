...
from db.session import get_db_admin, get_db_oper
from models.schedule import Agenda, Doctor
...
def create_schedule(
        ...
        db_oper: Session = Depends(get_db_oper)
):
    ...
    if not doctor_exists:
        raise HTTPException(status_code=404, detail="El médico no existe en la base de datos administrativa")

    # 2. Búsqueda de solapamientos en la DB Operativa para el mismo médico y fecha
    ...
    # 3. Creación del registro en la tabla 'agenda' de la DB Operativa
...
def get_schedules(id_agenda: Optional[int] = None, db_oper: Session = Depends(get_db_oper)):
...
def get_doctor_schedules(id_doctor: int, db_oper: Session = Depends(get_db_oper)):
...
def update_schedule(
    ...
    # Validación de solapamiento excluyendo el registro que se intenta actualizar
...
def delete_schedule(id_agenda: int, db_oper: Session = Depends(get_db_oper)):
    db_agenda = db_oper.query(Agenda).filter(Agenda.id_agenda == id_agenda).first()
    if not db_agenda:
        raise HTTPException(status_code=404, detail="No existe el registro")

    # Se opta por eliminación física, aunque podría cambiarse a estado=0 si se requiere auditoría
...
def generate_slots(
    ...
    for block in agendas:
        ...
        # Generación de intervalos basados en la duración parametrizada
